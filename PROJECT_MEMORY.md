...
| `polars-contract` | A2 makes Polars the default table type regardless of input; `.to_pandas()` explicitly converts. Inputs align by position, including named Series controls. Explicit categorical orders survive filtering; ordinary string categories sort observed levels. Native use does not install/import pandas; upstream binsreg still uses pandas internally. | [Contract](docs/INPUT_OUTPUT_CONTRACT.md), [parity tests](tests/test_polars_contract.py), [native gate](validation/native_smoke.py) | 2026-09-12 |
...
The [plan](binspect-plan.md) owns ordering and qualification status. Earlier
correctness/inference work is integrated through PRs #6–#15. The A1–P2
implementation stack is [PR #16](https://github.com/joshuamyers22/binspect/pull/16)
through [PR #23](https://github.com/joshuamyers22/binspect/pull/23): result ownership,
Polars and evidence exports, compatibility documentation, executable guide, figure
checks, gallery, occupied-cluster aggregation, and dependency configurations.
The [stack review](docs/pr-stack-review.md) records the categorical precision fix,
compatibility formatting correction, renderer CI correction, and actual checks.
GitHub PR records establish merge status; implementation delivery does not close
all milestone acceptance criteria.

Next implementation: P3 supply-chain and secret checks. M2 maintainer acceptance,
compatibility/baseline policy decisions, figure accessibility/visual qualification,
accepted performance budgets/runner, C3 qualified acceptance/final assessment,
and governance/security/release gates remain open. Few-cluster coverage remains
unsupported. Merge authorization is not independent statistical approval or a
release.
