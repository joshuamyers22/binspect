...
| Build/artifact identity | Hatchling 1.32.0, build 1.6.1 and Twine 7.0.0 are locked. No isolated backend re-resolution occurs in the audited build. Exactly one wheel and sdist must pass metadata checks; names, versions and library bytes (including `py.typed`) match the checkout, and the sdist manifest matches. Unsafe links/traversal, duplicate members and oversized archives fail. |
