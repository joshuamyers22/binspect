...
| T4 | Unsupported covariance or mixed group coordinates produce credible but misleading output. | C1 explicit scope; C2 rejects shared adjusted coordinates and preserves interval identity; C3 exposes inference limits and [records adjusted-bin undercoverage](inference-contract-review.md). | Shared adjustment remains unsupported; adjusted-bin nominal population coverage is unvalidated. C3 method/expanded-scenario decisions and qualified review remain open; C4 accurate diagnostics. |
