...
| T4 | Unsupported covariance or mixed group coordinates produce credible but misleading output. | C1 explicit scope; C2 rejects shared adjusted coordinates and preserves interval identity; C3 [withholds adjusted-bin uncertainty](adjusted-inference-boundary-review.md), rejects numerically unidentified controlled x, and [records uneven-cluster undercoverage](expanded-coverage-review.md). | Shared adjustment and adjusted-bin uncertainty remain unsupported. Uneven-cluster risk is not accepted by passing arithmetic checks; qualified support-policy review and locked assessment remain open; C4 accurate diagnostics. |
...
| T7 | Modified dependency/action/build tooling substitutes a malicious artifact. | P3 locks build/audit tools, pins workflow actions and adds advisory/license/history/artifact/SBOM checks. Six exact license scopes are approved through 2026-10-13; changed/expired scope blocks its gate. | [P3 scope](SUPPLY_CHAIN.md); R1 handoff/publisher qualification and actual controls; R2 provenance and release acceptance. |
